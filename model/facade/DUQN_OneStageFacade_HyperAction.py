...
from model.facade.DUQN_OneStageFacade import DUQN_OneStageFacade

class DUQN_OneStageFacade_HyperAction(DUQN_OneStageFacade):
    '''
    ...
    def parse_model_args(parser):
        ...
        - empty_start_rate
        '''
        parser = DUQN_OneStageFacade.parse_model_args(parser)
        return parser
...
#                 self.noise_var -= self.noise_decay
    ...
    def infer_hyper_action(self, observation, policy_output, actor, do_explore = False, do_softmax = True):
        '''
        ...
        with torch.no_grad():
            ...
            out_dict['action_emb'] = Z
            if 'candidate_ids' in observation:
                out_dict['candidate_features'] = observation['candidate_features']
                out_dict['candidate_ids'] = observation['candidate_ids']
                batch_wise = True
            else:
                out_dict['candidate_features'] = self.candidate_features.unsqueeze(0)
                out_dict['candidate_ids'] = self.candidate_iids
                batch_wise = False  
            action_prob = actor.score(out_dict['action_emb'], 
                                            out_dict['candidate_features'], 
                                            do_softmax = do_softmax)
            # two types of greedy selection
            if do_explore and np.random.rand() >= self.topk_rate:
                ...
                if batch_wise:
                    ...
                    action = out_dict['candidate_ids'][indices].detach() # (B, slate_size) 
            out_dict['action_features'] = self.candidate_features[indices]                      
        return {'Z': Z, 'action_emb': Z, 'state_emb': policy_output['state_emb'], 'action_features': out_dict['action_features']}

    
    def apply_critic(self, observation, policy_output, user_response, critic_model):
        feed_dict = {"state_emb": policy_output["state_emb"], 
                     "action_emb": policy_output["action_emb"]}
        critic_output = critic_model(feed_dict,user_response)
